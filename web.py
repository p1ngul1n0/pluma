...
class url:
	...
	def verificaUrl(self):
		if len(re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+',self.url)) > 0:
			...
			print (" > {}Você inseriu uma URL inválida!".format(Fore.RED))
			return False
...
class scan:
	...
	def info(self):
		...
		try:
			try:
				print ("      REGISTRO: {}".format(self.ip_info['registrar']))
				print ("      NOME: {}".format(self.ip_info['name']))
				print ("      EMAILS(S):")
				...
				print ("      REGISTRO: {}".format(self.ip_info['registrant']))
				print ("      NOME: {}".format(self.ip_info['person']))
		...
		print ("{}   TÍTULO: {}".format(Fore.WHITE,self.conteudo.find('title').text))
		opcao1 = input('      DESEJA EXTRAIR MAIS INFORMAÇÕES?(Y/n)')
		if opcao1 == 'n':
			cando1 = False
		...
		while cando1:
			...
			print ("{}   PALAVRAS CHAVE".format(Fore.WHITE))
			...
			for tag in self.emails:
				print ('     - {}'.format(tag))
			print ("{}   COMENTÁRIOS [{}]".format(Fore.WHITE,len(self.comentarios)))
			for tag in self.comentarios:
				print ('     - {}'.format(tag))
			print ("{}   RESPOSTA HTTP".format(Fore.BLUE))
			print ("-------------------------------------------------------")
	...
	def cookies(self):
		...
		if cabecalho in self.headers:
			...
			print ("     NÃO FORAM IDENTIFICADOS COOKIES")
	def methods(self):
	...
	def autocomplete(self):
		...
		print ("   {} CAMPO(S) DE ENTRADA DE SENHA ENCONTRADO(S).".format(len(inputs)))
		for input in inputs:
	...
	def enum(self):
		...
		opcao = input('      REALIZAR VARREDURA DE DIRETÓRIOS?(Y/n)')
		if opcao == 'n':
			cando = False
		...
		while cando:
			...
			num_403 = 0
			outros = 0
			print ("      UTILIZANDO ARQUIVO '{}'".format(arquivo))
			print ("      REALIZANDO VARREDURA DE {} DIRETÓRIO(S).".format(len(diretorios)))
			for diretorio in diretorios:
				...
				if response.status_code == 200:
					num_200 += 1
					print ('{}      #{} {} [{}] {} TAMANHO: {}'.format(Fore.RED,i,new_url,response.status_code,response.reason,len(response.content)))
					...
					print ('{}      #{} {} [{}] {} TAMANHO: {}'.format(Fore.YELLOW,i,new_url,response.status_code,response.reason,len(response.content)))
					num_403 += 1
				else:
					outros += 1
				i += 1
